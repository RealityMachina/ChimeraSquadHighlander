class XComHumanPawn extends XComUnitPawn native(Unit);
...
struct native PawnContentRequest
{
...
var transient bool bLoadVoice;
// bsg-dforrest(2.2.17): end
...
simulated function RequestFullPawnContent()
{
	...
	if (m_bSetAppearance)
	{
		...
		UpdateAllMeshMaterials(); // DIO - there is only the mesh, no additional components
		...
		// bsg-dforrest(2.2.17): optionally don't load pawn voice data, disabled for dropship matinee
		if (m_kAppearance.nmVoice != '' && `TACTICALGRI != none && bLoadVoice) //Only load the voices for tactical. In strategy play them on demand
		{
...
simulated function UpdateArmorMaterial(MeshComponent MeshComp, MaterialInstanceConstant MIC, int iPrimaryTint, int iSecondaryTint, XComPatternsContent Pattern )
{
	...
	{
		Palette = `CONTENT.GetColorPalette(ePalette_ArmorTint);
		if (Palette != none && iPrimaryTint < Palette.Entries.Length)
		{
...
simulated event bool GetTintColorsDIO( out LinearColor PrimaryColor, out LinearColor SecondaryColor )
{
	local XComLinearColorPalette Palette;
	local XComLinearColorPaletteEntry ArmorTint;
	local XComLinearColorPaletteEntry ArmorTintSecondary;
	...
	bHasStoredTint = ((BaseArmorTint.Primary.R + BaseArmorTint.Primary.G + BaseArmorTint.Primary.B) > 0.0f);
	bHasTintToApply = false;
	if( m_kAppearance.iArmorTint == INDEX_NONE && bHasStoredTint)
	{
		ArmorTint = BaseArmorTint;
		bHasTintToApply = true;
	}
	else if(m_kAppearance.iArmorTint != INDEX_NONE)
	{
		Palette = `CONTENT.GetColorPalette(ePalette_ArmorTint);
		if (Palette != none)
		{
			ArmorTint = Palette.Entries[m_kAppearance.iArmorTint ];
			if(m_kAppearance.iArmorTintSecondary != INDEX_NONE)
				ArmorTintSecondary = Palette.Entries[m_kAppearance.iArmorTintSecondary];
	...
	if (bHasTintToApply)
	{
		PrimaryColor = ArmorTint.Primary;
		SecondaryColor = ArmorTintSecondary.Secondary;
		return true;
	}
...
simulated function UpdateFlagMaterial(MaterialInstanceConstant MIC)
{
	...
	if (m_kAppearance.nmFlag != '')
	{
		...
		{
			//dakota.lemaster: disabling redscreen, we no longer require flag support
...
/*simulated function SetArmorTint(int TintIdx)
{
	...
	m_kAppearance.iArmorTintSecondary = TintIdx;
	UpdateAllMeshMaterials();               
}*/
...
{	
	...
	if(m_kAppearance.nmVoice != '')
	{
		...
		if(PartTemplate != none)
		{
			...
			// bsg-dforrest (7.20.17): do this async on consoles
			if(`ISCONSOLE)
			{
...
{
	// bsg-dforrest (7.20.17): when previewing lots of voices in the character pool unload banks for old voices
	// otherwise we will use up all our reduced wwise pool on console
	local XComCharacterVoice OldVoice;
	OldVoice = Voice;
	// bsg-dforrest (7.20.17): end
	...
	{
		if(	OldVoice != Voice && 
			`SCREENSTACK.IsInStack(class'UICustomize_Voice'))
		{
			if(OldVoice != none)
			{
				OldVoice.UnloadVoiceBank();
			}
		}
	}
	// bsg-dforrest (7.20.17): end
...
simulated function PrepForMatinee()
{
	...
	SetAuxParameters(true, false, false, false);
}
...
simulated function AsyncLoadAppearanceArchetype()
{
	...
	if (UnitState == none)
	{
		`log(GetFuncName()$" "$self$" FAILED UnitState == none!",, 'DevAsyncPawnLoading');
		return;
	}
			
	UnitState.RequestResources(ArchetypeNames);
...
reliable client function UnitSpeak( Name nEvent, optional XComGameStateContext Context = none)
{
	if (Voice != none)
	{
		`log(self.Name$".UnitSpeak: Playing sound for" @ nEvent, , 'DevSound');
		Voice.PlaySoundForEvent(nEvent, self, Context);
	}
	else
	{
...
function PlayHQIdleAnim(optional name OverrideAnimName, optional bool bIsCapture = false, optional bool bIgnoreInjuredAnim = false)
{
	...
	local DioStrategyCrewMgr CachedCrewMgr;

	if (bPhotoboothPawn) return;

	//Skip if we are in the armory, it has its own set of anims
	CachedCrewMgr = `STRATEGYRULES.CrewMgr;
	if (CachedCrewMgr.bInArmory)
	{
		return;
	}
...
`endif
		...
		// bsg-dforrest (4.11.17): support to async load appearance updates, forward new param
		super.SetAppearance(kAppearance, bRequestContent, bAsyncLoad);
		// bsg-dforrest (4.11.17): end
	}
...
simulated event XComCharacterVoice GetVoice()
{
	return Voice;
}
...
defaultproperties
{
	...
	bLoadVoice=true // bsg-dforrest(2.2.17): optionally don't load pawn voice data, disabled for dropship matinee
}
