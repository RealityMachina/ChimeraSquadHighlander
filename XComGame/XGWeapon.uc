...
// Issue #7 (WOTC CHL #246) deprivatize PatternsContent
...
simulated function Actor CreateEntity(optional XComGameState_Item ItemState=none)
{
	...
	local XComHumanPawn OwnerHumanPawn;
	local int i;
	...
	if(Role == ROLE_Authority)
	{
		...
		{
			OwnerHumanPawn = XComHumanPawn(kOwner);			

			if (OwnerHumanPawn != none)
			{
				strArchetype = WeaponTemplate.DetermineGameArchetypeForUnit(InternalWeaponState, UnitState, OwnerHumanPawn.m_kAppearance);
			}
			else
			{
...
simulated function AsyncLoadAppearanceArchetype()
{
	...
	if( UnitState != none )
	{
		...
		if (PartTemplate != none)
		{
			...
			UnitState.LoadArchetypesByListAsync(self, ArchetypeNames, OnWeaponAppearanceLoaded);
	...
	{
		`log(GetFuncName()$" "$self$" FAILED m_kOwner == none! Loading immediately.",, 'DevAsyncPawnLoading');
		// can't do an async update if we can't find the object
...
simulated function SetAppearance( const out TWeaponAppearance kAppearance, optional bool bRequestContent=true, optional bool bAsyncLoad=false )
// bsg-dforrest (4.11.17): end
{
