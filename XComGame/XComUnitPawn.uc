...
var bool bPlayedBodyFallImpactSound;
				...
				const out CollisionImpactData RigidCollisionData, int ContactIndex )
{
	...
	if (fFallImpactSoundEffectTimer > 0.05f && !bPlayedBodyFallImpactSound)   // these values were found empirically. The timer is used for general purpose now.
	{
		bPlayedBodyFallImpactSound = true;
...
simulated function PlayHitEffects(float Damage, Actor InstigatedBy, vector HitLocation, name DamageTypeName, vector Momentum, bool bIsUnitRuptured, EAbilityHitResult HitResult= eHit_Success, optional TraceHitInfo ThisHitInfo )
{
	...
	if (HitEffect != None && HitResult != eHit_Shield)
	{
...
simulated function PlayDying(class<DamageType> DamageTypeClass, vector HitLoc, optional Name AnimName='', optional vector Destination)
{
	...
	for(Index = 0; Index < UnitState.KilledByDamageTypes.Length; ++Index)
	{
		...
		{
			if (!DamageTypeTemplate.bAllowAnimatedDeath)
			{
				if (DeathAction == none	|| !DeathAction.OverrideAllowAnimatedDeathForDamageType(DamageTypeTemplate.DataName))
				{
					bRagdollImmediately = true;
				}
			}
		}
	}
	...
	m_bAuxParamUseHiddenOutline = false;

	bScanningProtocolOutline = false;
	bTargetDefinitionOutline = false;
	SetAuxParameters(m_bAuxParamNeedsPrimary, m_bAuxParamNeedsSecondary, m_bAuxParamUse3POutline, m_bAuxParamUseHiddenOutline);

	//Don't mark the unit "dead in visualizer" unless they're really dead (would prevent AOE-marking incapacitated units)
	if (UnitState.IsDead())
	{
		LightRig.SetEnabled(false);
							 ...
							 bool DropWeapon = true)
{
	...
	if(DropWeapon && m_bDropWeaponOnDeath && !bDroppedWeapon)
	{
	...
	bIsSoldier = UnitState.GetMyTemplate().bTreatAsSoldier;
	...
	{	
		...
		SetTimer(0.2f, false, nameof(DelayedTargetPawnSlomo));
	}

	GotoState('RagDollBlend');
}

function DelayedTargetPawnSlomo()
{
	...
	BreachActionSequencer.AddObjectToBreachFireSlomoGroup(m_kGameUnit, BreachActionSequencer.SlomoGroupHandle_TargetPawns, 0.0f);
}
...
function SetNoSkeletonUpdate(bool bSetting)
{
	...
	//`warn("Time[" @ WorldInfo.TimeSeconds @ "] SetNoSkeletonUpdate for" @ self @ "to" @ bSetting);
...
simulated function GameStateResetVisualizer(XComGameState_Unit UnitState)
{
	...
	`log("Setting" @ self @ "position to tile: (" @ UnitState.TileLocation.X @ "," @ UnitState.TileLocation.Y @ "," @ UnitState.TileLocation.Z @ ")",,'XCom_Visualization');

	NewLocation.Z = m_kGameUnit.GetDesiredZForLocation(NewLocation, `XWORLD.IsFloorTile(UnitState.TileLocation));

	`log("Setting" @ self @ "position to Location:" @ NewLocation, , 'XCom_Visualization');
...
function ResetRagdoll()
{
	Mesh.WakeRigidBody();
	SetNoSkeletonUpdate(false);
	bAnimatedRagdoll = true;
	fPhysicsMotorForce = 10.0;	
	ClearTimer('FinishRagdoll');
	SetTimer(20.0f, false, 'FinishRagDoll');
}
...
simulated state RagDollBlend
{
	...
	{	
		...
		// if we're doing latent submission, defer the FinishRagDoll until the next frame where we're clear --Ned
		// Also if the breach action sequencer slomo is still active
		if (`XCOMGAME.GameRuleset.IsDoingLatentSubmission() || !BreachActionSequencer.CanFinishRagdoll())
		{
			SetTimer(RagdollFinishTimer, false, 'FinishRagDoll');
			return;
		}
	...
	{		
		...
		if (fPhysicsMotorForce != 0.0f)
		{
			...
			{
				if(!bWaitingForRagdollNotify && !BreachActionSequencer.ShouldApplyBreachFireSlomo())
				{
...
simulated event PostInitAnimTree(SkeletalMeshComponent SkelComp)
{
	...
	SetSwitch('Dio_Characters', AkEventCharacterSwitch);
...
simulated function array<XGInventoryItem> CreateVisualInventoryAttachments(UIPawnMgr PawnMgr, XComGameState_Unit UnitState, optional XComGameState CheckGameState, bool bSetAsVisualizer=true, bool OffsetCosmeticPawn=true, bool bUsePhotoboothPawns=false, bool bArmorAppearanceOnly=false)
{
	local array<AnimSet> PhotoboothAnimSets;
	local array<XGInventoryItem> Items;
	...
	//Start at the primary weapon slot and work our way to MAX - creating items as needed
	for (Index = eInvSlot_PrimaryWeapon; Index < eInvSlot_MAX; ++Index)
	{
		Slot = EInventorySlot(Index);
		Item = CreateVisualInventoryAttachment(PawnMgr, Slot, UnitState, CheckGameState, bSetAsVisualizer, OffsetCosmeticPawn, bUsePhotoboothPawns, PhotoboothAnimSets, bArmorAppearanceOnly);
		if (Item != none)
		{
			Items.AddItem(Item);
		}
...
{
	m_kAppearanceBase.iArmorTint = TintIdx;
	m_kAppearanceBase.iArmorTintSecondary = TintIdx;
...
private function OnAppearanceLoaded(XComGameState_Unit Unit)
{
	local XComEngine GameEngine; // bsg-dforrest (7.25.17): move from LoadArchetypesByListAsync

	`log(GetFuncName()$" "$self,, 'DevAsyncPawnLoading');
	SetTimer(0.01f, false, nameof(RequestFullPawnContent), self); // do the full request off the timer (during actors ticking)

	// bsg-dforrest (7.25.17): move from LoadArchetypesByListAsync
	GameEngine = XComEngine(class'Engine'.static.GetEngine());
	GameEngine.m_asyncPawnLoadRequestCount--;
	// bsg-dforrest (7.25.17): end
}

simulated function AsyncLoadAppearanceArchetype()
{
	local XComGameState_Unit UnitState;
	local array<string> ArchetypeNames;
	...
	if (m_kGameUnit == none)
	{
		`log(GetFuncName()$" "$self$" FAILED m_kGameUnit == none!",, 'DevAsyncPawnLoading');
		return;
	}

	// get the owner unit state
	UnitState = XComGameState_Unit(`XCOMHISTORY.GetGameStateForObjectID(m_kGameUnit.ObjectID));
	if( UnitState != none )
	{
		...
		{
			ArchetypeNames.AddItem(PartTemplate.ArchetypeName);
			UnitState.LoadArchetypesByListAsync(self, ArchetypeNames, OnAppearanceLoaded);
...
// bsg-dforrest (4.11.17): support to async load appearance updates
simulated function SetAppearance( const out TAppearance kAppearance, optional bool bRequestContent=true, optional bool bAsyncLoad=false )
// bsg-dforrest (4.11.17): end
{
	m_kAppearanceBase = kAppearance;
	if (bRequestContent)
	{
		// bsg-dforrest (4.11.17): support to async load appearance updates, added bAsyncLoad
		if( bAsyncLoad )
		{
			AsyncLoadAppearanceArchetype();
		}
		else
		{
...
simulated function XGInventoryItem CreateVisualInventoryAttachment(UIPawnMgr PawnMgr, EInventorySlot InvSlot, XComGameState_Unit UnitState, XComGameState CheckGameState, bool bSetAsVisualizer, bool OffsetCosmeticPawn, bool bUsePhotoboothPawns = false, optional out array<AnimSet> PhotoboothAnimSets, bool bArmorAppearanceOnly = false)
{
...
{	
	...
	simulated event BeginState(Name PreviousStateName)
	{
		...
		MarkAuxParametersAsDirty(TRUE, FALSE, FALSE, FALSE);
		m_bAuxParamNeedsPrimary = TRUE;
		m_bAuxParamNeedsSecondary = FALSE;
		m_bAuxParamUse3POutline = FALSE;
		m_bAuxParamUseHiddenOutline = FALSE;
		...
		{
			kPawnPerkShared.OnPawnDeathOfPerkTarget( XGUnit(m_kGameUnit) );
		}
...
function SetUpdateSkelWhenNotRendered(bool bSetting)
{
	...
	if (!bSetting)
	{
		...
		if (UnitState != None)
		{
			...
			if (CharacterTemplate.bTreatAsSoldier)
			{
				//`log("ignored the request that tried to turn off [bUpdateSkelWhenNotRendered] for XComUnit" @ string(CharacterTemplate.DataName));
				return;
			}
		}
		
		//Physics needs to be able to update the ragdoll
		if (GetStateName() == 'RagdollBlend')
		{
			return;
		}
 ...
 */
 // FIRAXIS UPDATE 4.11.2019 - rmcfall - Support for FaceFX 2018 / Runtime 1.3 - Conformed PlayFaceFX calls to new API
event bool PlayActorFaceFXAnim(FaceFXAnim Anim, AkEvent AkEventToPlay)
{	
	if (m_kHeadMeshComponent != none)
	{
		return m_kHeadMeshComponent.PlayFaceFXAnim(Anim, AkEventToPlay);
	}
	return Mesh.PlayFaceFXAnim(Anim, AkEventToPlay);
}
...
simulated function DebugAims(Canvas kCanvas, XComCheatManager kCheatManager)
{	
	local Vector FireSocketLoc;
	local Rotator TrueAim;
	local Vector TrueAimLineEnd;
	local Vector vScreenPosMidPoint;
	local Vector vScreenPosTrueAimLineEnd;
	local Vector vScreenPos;
	local string UnitInfo;
	local int VerticalSpacing;
	local XComGameState_Unit UnitState;

	if (kCheatManager.m_bDebugAims)
	{
		UnitState = XComGameState_Unit(`XCOMHISTORY.GetGameStateForObjectID(m_kGameUnit.ObjectID));
		if (kCheatManager.m_DebugAimsTeam == eTeam_All || UnitState == none || kCheatManager.m_DebugAimsTeam == UnitState.GetTeam())
		{
			GetAimSocketOrBone(FireSocketLoc, TrueAim);
			if (bLockAiming)
			{
				`SHAPEMGR.DrawLine(FireSocketLoc, TargetLoc, 1, MakeLinearColor(1.0f, 0.0f, 0.0f, 1.0f));
			}
			else
			{
				`SHAPEMGR.DrawLine(FireSocketLoc, TargetLoc, 1, MakeLinearColor(0.0f, 1.0f, 0.0f, 1.0f));
			}

			VerticalSpacing = 12.0f;

			//Draw the direction the fire node socket is aiming - represents the barrel of the weapon
			TrueAimLineEnd = FireSocketLoc + (Vector(TrueAim) * 100);
			`SHAPEMGR.DrawLine(FireSocketLoc, TrueAimLineEnd, 1, MakeLinearColor(0.0f, 1.0f, 1.0f, 1.0f));
			vScreenPosTrueAimLineEnd = kCanvas.Project(TrueAimLineEnd);
			vScreenPosTrueAimLineEnd.Y += VerticalSpacing;
			kCanvas.SetPos(vScreenPosTrueAimLineEnd.X, vScreenPosTrueAimLineEnd.Y);
			kCanvas.SetDrawColor(0, 255, 255);
			kCanvas.DrawText("GetAimSocketOrBone Direction");

			// Identify the primary aim line - do this by printing over the midpoint. End points are more likely to overlap with enemy aims
			UnitInfo = "Unit:" @ self @ "(Archetype:" @ self.ObjectArchetype @ ")" @ "(ObjectID:" @ self.ObjectID @ ")";

			vScreenPosMidPoint = kCanvas.Project((FireSocketLoc + TargetLoc) / 2.0);
			vScreenPosMidPoint.Y += VerticalSpacing;
			kCanvas.SetPos(vScreenPosMidPoint.X, vScreenPosMidPoint.Y);
			if (bLockAiming)
			{
			...
			kCanvas.DrawText(UnitInfo);

			//Aim lock information
			vScreenPos = kCanvas.Project(Location);
			vScreenPos.Y += VerticalSpacing;
			kCanvas.SetPos(vScreenPos.X, vScreenPos.Y);
			kCanvas.SetDrawColor(255, 255, 255);
			kCanvas.DrawText("bLockAiming:");
			kCanvas.SetPos(vScreenPos.X + 70, vScreenPos.Y);
			if (bLockAiming)
			{
				kCanvas.SetDrawColor(255, 0, 0);
				kCanvas.DrawText("TRUE");
			}
			else
			{
				kCanvas.SetDrawColor(0, 255, 0);
				kCanvas.DrawText("FALSE");
			}

			//Display info identifying the units so that the lines can be matched to them
			`SHAPEMGR.DrawSphere(Location, vect(3, 3, 3), MakeLinearColor(1, 1, 1, 1), false);
			vScreenPos.Y += VerticalSpacing;
			kCanvas.SetPos(vScreenPos.X, vScreenPos.Y);
			kCanvas.SetDrawColor(255, 255, 255);
			kCanvas.DrawText(UnitInfo);
			...
			kCanvas.SetPos(vScreenPos.X, vScreenPos.Y);
			kCanvas.SetDrawColor(255, 255, 255);
			kCanvas.DrawText("AimOffset:");

			kCanvas.SetPos(vScreenPos.X + 70, vScreenPos.Y);
			kCanvas.SetDrawColor(255, 255, 255);
			kCanvas.DrawText( "(" $ AimOffset.X $ ", " $ AimOffset.Y $ ")" );
		}
	}
}
...
simulated function DebugVis( Canvas kCanvas, XComCheatManager kCheatManager )
{
	...
	if( kCheatManager.m_bDebugVis )
	{
		...
		kCanvas.SetPos(vScreenPos.X, vScreenPos.Y += 15.0f);
		kCanvas.DrawText("RoomID:"@UnitState.AssignedToRoomID);

		// Room Traversal
		kCanvas.SetPos(vScreenPos.X, vScreenPos.Y += 15.0f);
		kCanvas.DrawText("CanTraverseRoom:"@UnitState.bCanTraverseRooms);

		// Alert Level
		kCanvas.SetPos(vScreenPos.X, vScreenPos.Y += 15.0f);
		kCanvas.DrawText("AlertLevel:"@ m_kGameUnit.GetAlertLevel());

		kCanvas.SetPos(vScreenPos.X, vScreenPos.Y += 15.0f);
		kCanvas.DrawText("VisAlertLevel:"@ m_kGameUnit.GetVisualAlertLevel());

		// voice id
		kCanvas.SetPos(vScreenPos.X, vScreenPos.Y += 15.0f);
		kCanvas.DrawText("Voice ID:"@ UnitState.AssignedVoiceID);

		kCanvas.SetPos(vScreenPos.X, vScreenPos.Y += 15.0f);
		kCanvas.DrawText("Tick Anim:"@ Mesh.bUpdateSkelWhenNotRendered);
	}
}
...
`if (`notdefined(FINAL_RELEASE))
	...
	if (kCheatManager != none)
	{
		...
		DebugAims(kCanvas, kCheatManager);
